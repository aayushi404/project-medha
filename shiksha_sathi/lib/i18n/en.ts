/**
 * English UI strings. This object also defines the shape (`Copy`) that every
 * other locale must match. The backend returns generated content in the
 * teacher's `preferred_language`; this file is only the app chrome.
 */
export const en = {
  brand: "Medha",
  nav: {
    home: "Home",
    askMedha: "Ask Medha",
    history: "History",
    modules: "My Modules",
    tools: "Tools",
    attendance: "Attendance",
    workUpdate: "Work Update",
    notifications: "Notifications",
    students: "Students",
    homework: "Homework",
    timetable: "Timetable",
    reportCard: "Report Card",
    resources: "E-Library",
    notes: "Notes",
    practice: "Practice",
    simulations: "Simulations",
  },
  navMain: "Main",
  studentNav: {
    ask: "Ask Medha",
    practice: "Practice",
    notes: "Notes",
    library: "Library",
    english: "Learn English",
    homework: "Homework",
    timetable: "Timetable",
    reportCard: "Report Card",
    resources: "E-Library",
    fees: "Fees",
    biharDarpan: "Bihar Darpan",
    simulations: "Simulations",
    codingHub: "Coding Hub",
    skillsHub: "Skills",
    attendance: "Attendance",
  },
  profileMenu: { edit: "Edit profile", logout: "Log out" },
  language: "Language",
  askMedhaBarPlaceholder: "Ask Medha AI anything…",
  askMedhaBarMic: "Ask by voice",

  sidebar: {
    quote: "शिक्षा से समृद्ध बिहार, समृद्ध भारत",
    nalanda: "Nalanda University",
  },
  dashboard: {
    heroTagline: "बड़े सपने शिक्षा के साथ",
    quote: "हर बच्चे में सीखने की अनंत संभावना है।",
    quoteAttrib: "– मेधा",
    notifications: "Notifications",
  },
  lessonPlan: {
    wizard: {
      title: "Lesson plan",
      backToHome: "Back to home",
      stepTopics: "Topics",
      stepSetup: "Lesson setup",
      selectClass: "Select Class",
      selectSubject: "Select Subject",
      chapters: "Chapters",
      chaptersHint: "Select one chapter.",
      noChapters: "No chapters found for this class and subject.",
      continue: "Continue",
      back: "Back",
      periodsLabel: "Number of Periods",
      periodOption: (n: number) => `${n} period${n === 1 ? "" : "s"}`,
      objectiveLabel: "Lesson objective (optional)",
      objectivePlaceholder:
        "Share how you'd like this lesson to be taught (e.g., through activities, examples, or skills-based learning).",
      generate: "Generate lesson",
      generating: "Generating your lesson plan…",
    },
  },
  quizWizard: {
    title: "Quiz",
    backToHome: "Back to syllabus",
    stepTopics: "Topics",
    stepSetup: "Quiz settings",
    selectClass: "Select Class",
    selectSubject: "Select Subject",
    chapters: "Chapters",
    chaptersHint: "Select one chapter.",
    noChapters: "No chapters found for this class and subject.",
    continue: "Continue",
    back: "Back",
    questionCount: "Question count",
    custom: "Custom",
    level: "Level",
    levelEasy: "Easy",
    levelStandard: "Standard",
    levelHard: "Hard",
    totalTime: "Total time for students",
    minutes: (n: number) => `${n} min`,
    objectiveLabel: "Quiz objective (optional)",
    objectivePlaceholder:
      "Tell Medha what you'd like this quiz to test — for example, basic understanding, application, or deeper thinking.",
    generate: "Generate quiz",
    generating: "Generating your quiz…",
  },
  questionPaperWizard: {
    title: "Question paper",
    backToHome: "Back to question paper",
    stepTopics: "Topics",
    stepSetup: "Paper setup",
    selectClass: "Select Class",
    selectSubject: "Select Subject",
    chapters: "Chapters",
    chaptersHint: "Select one chapter.",
    noChapters: "No chapters found for this class and subject.",
    continue: "Continue",
    back: "Back",
    marksAndQuestions: "Marks & questions",
    marksHint: "Total marks, difficulty, and counts per type.",
    difficulty: "Difficulty",
    diffEasy: "Easy",
    diffMedium: "Medium",
    diffHard: "Hard",
    diffMixed: "Mixed",
    totalMarks: "Total marks",
    totalQuestions: "Total questions",
    addType: "Add type",
    typeMcq: "MCQs",
    typeVeryShort: "Very Short Answer",
    typeShort: "Short Answer",
    typeLong: "Long Answer",
    typeCaseStudy: "Case study",
    questionsField: "Questions",
    marksEach: "Marks each",
    objectiveLabel: "Question paper objective (optional)",
    objectivePlaceholder:
      "Tell Medha how this paper should feel — for example, exam-style balance, case-based, or board pattern.",
    generate: "Generate paper",
    generating: "Generating your question paper…",
  },

  // dashboard
  greeting: (name: string) => (name ? `Hello, ${name}` : "Hello"),
  greetingSub: "What are you teaching today?",
  inputPlaceholder: "How do I teach photosynthesis so students stay engaged?",
  needContext: "Pick a class and subject first.",
  selectClass: "Class",
  selectSubject: "Subject",
  selectChapter: "Chapter",
  quickActions: {
    explanation: { label: "Teaching approach", sub: "2-3 strategies" },
    quiz: { label: "Quiz", sub: "Questions to ask" },
    activity: { label: "Class activity", sub: "Low-tech, hands-on" },
    ppt: { label: "Slides", sub: "Editable .pptx deck" },
    mindmap: { label: "Mindmap", sub: "Coming soon" },
  },
  explanationPrompt: "How do I teach this topic in an engaging way?",
  makeQuiz: "Make a quiz",
  makeActivity: "Make an activity",
  makeMindmap: "Mind map",
  makePpt: "Slides",
  comingSoonMindmap: "Mind maps are coming soon.",
  deckDownload: "Download .pptx",
  deckSlides: (n: number) => `${n} slide${n === 1 ? "" : "s"}`,
  readyMadeSlides: "Ready-made slides for this chapter",
  readAloud: "Read aloud",
  stopReading: "Stop",
  generating: "Generating…",
  streamError: "The response didn't finish. Please try again.",
  retry: "Retry",

  // /ask empty-state hero
  askHero: {
    eyebrow: "Your AI Teaching Assistant",
    greeting: (name: string) => (name ? `Welcome, ${name}` : "Welcome"),
    subtitle: "Ask me anything, or set up your class below to get started.",
    classSubjectTitle: "Class & subject",
    classSubjectPlaceholder: "Pick what you teach",
    chapterTitle: "Chapter",
    chapterPlaceholder: "Optional — for tailored answers",
  },

  // chapter history (dashboard)
  chapterHistoryTitle: "Earlier in this chapter",
  chapterHistoryHint: "Anything you generate below is saved here.",
  chapterHistoryEmpty:
    "Nothing generated for this chapter yet. Ask a question or use a quick action below.",
  chapterHistoryFailed: "Couldn't load earlier content for this chapter.",
  pickChapterForHistory: "Pick a chapter to see what you've already made for it.",
  chapterHistoryCount: (n: number) => `${n} item${n === 1 ? "" : "s"}`,
  openFullModule: "Open full module",

  // my modules
  myModules: "My Modules",
  myModulesSub: "Everything you've made, arranged chapter by chapter.",
  filterAll: "All",
  searchPlaceholder: "Search by name",
  emptyModules: "No modules yet. Ask about a topic on the dashboard.",
  moduleNotFound: "This module wasn't found.",
  back: "Back",
  answerLabel: "Answer",
  materialsNone: "Nothing needed",

  // chapter browser
  pickClassSubject: "Pick a class you teach",
  noSubjects: "Add the classes you teach in your profile to see them here.",
  chaptersEmpty: "No chapters listed for this class yet.",
  chapterNothingYet: "Nothing here yet.",
  startLesson: "Start a lesson",
  expandAll: "Expand all",
  collapseAll: "Collapse all",
  otherModules: "Not linked to a chapter",
  moduleCount: (n: number) => `${n} module${n === 1 ? "" : "s"}`,
  noSearchHits: "No modules match that search.",
  chaptersStarted: (done: number, total: number) => `${done}/${total} chapters started`,
  chapterEmpty: "empty",

  // module detail
  allTypes: "All",
  sectionTitle: {
    explanation: "Explanation",
    quiz: "Quizzes",
    activity: "Activities",
    ppt: "Slides",
  } as Record<string, string>,
  revealAnswers: "Reveal answers",
  hideAnswers: "Hide answers",
  showAnswer: "Show answer",
  hideAnswer: "Hide answer",
  practiceHint: "Tap an option to check yourself",
  conversationTitle: "Original conversation",
  conversationSub: "The chat this module came from",
  conversationEmpty: "No messages saved for this module.",
  conversationFailed: "Couldn't load the conversation.",
  copyText: "Copy",
  copied: "Copied",
  feedbackUp: "Helpful",
  feedbackDown: "Could be better",
  commentPlaceholder: "Anything to add? (optional)",
  send: "Send",
  save: "Save",
  cancel: "Cancel",
  deleteModule: "Delete module",
  deleteConfirmTitle: "Delete this module?",
  deleteConfirmBody: "All of its artifacts will be removed too. This can't be undone.",
  confirmDelete: "Delete",

  artifactLabel: {
    explanation: "Explanation",
    quiz: "Quiz",
    activity: "Activity",
    ppt: "Slides",
  } as Record<string, string>,

  // student section
  student: {
    askTitle: "Ask Medha",
    askSub: "Pick a chapter and ask anything you don't understand.",
    askHeading: (name: string) =>
      name ? `Hi ${name}, what's your doubt?` : "Hi, what's your doubt?",
    askAnything: "Ask anything about this chapter.",
    pickToStart: "Pick a subject and chapter to start.",
    didntFinish: "That didn't finish. Try asking again.",
    askPlaceholder: "Ask your doubt about this chapter…",
    pickPlaceholder: "Pick a subject and chapter first",
    pickSubjectChapterToast: "Pick a subject and chapter first.",
    couldNotStart: "Couldn't start.",

    notesTitle: "Notes",
    notesSub: "Short, revision-ready notes for every chapter.",
    notesPickSubject: "Pick a subject to browse chapter notes.",
    notesChooseChapter: "Choose a chapter above.",
    keyPoints: "Key points",
    importantTerms: "Important terms",
    notesPreparing: (title: string) => `Notes for “${title}” are being prepared`,
    notesPreparingBody:
      "Each chapter will have a short summary, key points, explained sections and a list of important terms.",
    pdfSoon: "PDF download coming soon.",

    practiceTitle: "Practice",
    practiceSub: "Mock tests and chapter-wise questions to check how ready you are.",
    practicePickToStart: "Pick a subject and chapter to see practice sets.",
    practiceSampleNote:
      "Full question banks for this chapter are still being prepared — you'll see a short sample set for now.",
    practiceEmpty: "No practice questions for this chapter yet.",
    questionsCount: (n: number) => `${n} questions`,
    startBtn: "Start",

    libraryTitle: "Library",
    librarySub: "Textbooks, reference books, story books and more — all in one place.",
    searchBooks: "Search books",
    categoryAll: "All",
    noBooksMatch: "No books match that search.",
    pagesCount: (n: number) => `${n} pages`,
    downloadBtn: "Download",
    readOnlineBtn: "Read online",
    downloadSoon: "Download coming soon.",
    readerSoon: "Reader coming soon.",
    booksTab: "Books",
    presentationsTab: "Presentations",
    presentationsSub: "Slide decks for your class, ready to view and download.",
    searchPresentations: "Search presentations",
    noPresentations: "No presentations for your class yet.",
    slidesCount: (n: number) => `${n} slide${n === 1 ? "" : "s"}`,
  },

  voice: {
    title: "Talk to Medha",
    subtitle: "Speak naturally — Medha listens and replies aloud",
    openChat: "Open voice chat",
    tapToTalk: "Tap the mic and speak",
    listening: "Listening… tap again when done",
    thinking: "Medha is thinking…",
    speaking: "Medha is speaking…",
    startRecording: "Start voice input",
    stopRecording: "Stop recording",
    emptyHint: "Press the microphone and ask anything — in Hindi, English, or a mix.",
    autoListen: "Hands-free conversation",
    handsFreeStart: "Tap mic to start hands-free chat",
    handsFreeListening: "Speak naturally — I'll listen when you pause",
    transcribeFailed: "Could not understand that. Please try again.",
    recordFailed: "Recording failed. Please try again.",
    micUnavailable: "Microphone is not available on this device.",
    micDenied: "Microphone access was denied. Check browser permissions.",
  },

  english: {
    title: "Learn English",
    subtitle: "Practice speaking, vocabulary, and grammar with Medha",
    tabChat: "Talk",
    tabLessons: "Lessons",
    tabVocab: "Words",
    tabSpeak: "Speak",
    tabWord: "Word of the Day",
    pickLesson: "Pick a lesson to start",
    startLesson: "Start lesson",
    dailyWordTitle: "Today's word",
    dailyWordTip: "Pronunciation tip",
    vocabPractice: "Tap a word to practice with Medha",
    speakPrompt: "Read this aloud, then ask Medha to check your pronunciation",
    speakWithMedha: "Practice with voice",
    listenBtn: "Listen",
    recordBtn: "Record yourself",
    stopRecordBtn: "Stop",
    checkingPronunciation: "Checking…",
    scoreLabel: (n: number) => `Score: ${n}/100`,
    heardLabel: "We heard",
    tipsLabel: "Tips",
    chatPlaceholder: "Ask anything about English…",
    greeting: (name: string) =>
      name ? `Hi ${name}, ready to learn English?` : "Hi, ready to learn English?",
    greetingSub: "Pick a lesson or just start talking — Medha will guide you.",
  },

  // auth
  roleLabel: {
    admin: "an administrator",
    principal: "a principal",
    teacher: "a teacher",
    student: "a student",
  } as Record<string, string>,
  login: {
    subtitle: "Your AI teaching companion",
    email: "Email",
    emailPlaceholder: "you@example.com",
    password: "Password",
    passwordPlaceholder: "Your password",
    submit: "Log in",
    submitting: "Logging in…",
    couldNotLogIn: "Could not log in.",
    newToMedha: "New to Medha?",
    registerAsPrefix: "Register as a",
    rolePrincipal: "principal",
    roleTeacher: "teacher",
    roleStudent: "student",
    or: "or",
    studentApproved: "Student approved?",
    activate: "Activate your account",
    welcomeBack: (name: string) => `Welcome back, ${name}`,
    signedIn: "Signed in",
    signedInAsPre: "You're signed in as",
    signedInAsPost: ".",
    takingYou: "Taking you to your workspace…",
    pendingMessage:
      "Your account hasn't been approved yet. You'll be able to log in once it is.",
    rejectedTitle: "Registration not approved",
    rejectedReason: (r: string) => `Reason: ${r}`,
    rejectedFallback: "Your registration was not approved.",
    registerAgain: "Register again",
    backToLogin: "Back to log in",
    wrongPortal: (roleLabel: string, tabLabel: string) =>
      `This account is registered as ${roleLabel}. Please log in from the ${tabLabel} tab.`,
  },
  pending: {
    title: "Waiting for approval",
    body: (approver: string) =>
      `Your account has been created and is pending approval from ${approver}. You'll be able to log in once it's approved.`,
    backToLogin: "Back to log in",
  },

  // content generation (Medha v2)
  generation: {
    types: {
      lesson_plan: { label: "Lesson Plan", short: "Lesson plan" },
      presentation: { label: "Presentation", short: "Presentation" },
      question_paper: { label: "Question Paper", short: "Question paper" },
      quiz: { label: "Quiz", short: "Quiz" },
      notes: { label: "Notes", short: "Notes" },
    } as Record<string, { label: string; short: string }>,
    home: {
      subtitle: "What should we make today?",
      quickActionsTitle: "Quick Actions",
      createLabel: {
        lesson_plan: "Create Lesson Plan",
        presentation: "Create Presentation",
        question_paper: "Create Question Paper",
        quiz: "Create Quiz",
        notes: "Create Notes",
      } as Record<string, string>,
      createDesc: {
        lesson_plan: "Step-by-step teaching plan for your class",
        presentation: "Engaging slides for your class",
        question_paper: "Generate questions with answers",
        quiz: "Interactive quiz for students",
        notes: "Simple and clear notes",
      } as Record<string, string>,
      recentTitle: "Recent",
      viewAll: "View all",
      recentEmpty: "Nothing generated yet. Pick a card above to get started.",
      askMedhaTitle: "Ask Medha",
      askMedhaSub: "Talk it through first, then turn it into a document.",
      askMedhaCta: "Start a conversation",
    },
    create: {
      scopeRequired: "Pick a class and subject first.",
      generateBtn: "Generate",
      generating: "Generating…",
      buildingSlides: "Building your slides…",
      cancel: "Cancel",
      reusedEarlier: "Reused something you made earlier for this.",
      failed: "Couldn't generate that.",
      tryAgain: "Try again",
      params: {
        periods: "Periods",
        focus: "Focus / notes (optional)",
        focusPlaceholder: "Anything specific to emphasise…",
        slideCount: "Slides",
        detail: "Detail",
        detailSimple: "Simple",
        detailDetailed: "Detailed",
        includeNotes: "Include speaker notes",
        totalMarks: "Total marks",
        durationMin: "Duration (minutes)",
        mcqCount: "MCQ questions",
        shortCount: "Short questions",
        longCount: "Long questions",
        questionCount: "Questions",
        difficulty: "Difficulty",
        difficultyEasy: "Easy",
        difficultyMedium: "Medium",
        difficultyHard: "Hard",
        difficultyMixed: "Mixed",
        questionTypes: "Question types",
        typeMcq: "MCQ",
        typeShort: "Short answer",
        typeTruefalse: "True / False",
        depth: "Depth",
        depthSummary: "Summary",
        depthStandard: "Standard",
        depthDetailed: "Detailed",
        includeKeyTerms: "Include glossary",
      },
    },
    viewer: {
      favorite: "Favourite",
      unfavorite: "Favourited",
      download: "Download",
      downloadPptx: "Download .pptx",
      regenerate: "Regenerate",
      regenerating: "Regenerating…",
      print: "Print",
      delete: "Delete",
      deleteConfirmTitle: "Delete this?",
      deleteConfirmBody: "This can't be undone.",
      notFound: "This wasn't found.",
      failedTitle: "This generation failed.",
      lessonPlan: {
        period: "Period",
        concept: "Concept",
        objective: "Learning objective",
        outcomes: "Learning outcomes",
        process: "Teacher-learning process",
        assessment: "Assessment",
        resources: "Resources",
        homework: "Homework",
        teacher: "Teacher",
        topic: "Topic",
        periods: "Periods",
        edit: "Edit",
        save: "Save",
        saving: "Saving…",
        cancel: "Cancel",
        saved: "Lesson plan saved.",
        saveFailed: "Couldn't save your changes.",
        addPeriod: "Add period",
        removePeriod: "Remove period",
        download: "Download",
        downloadPdf: "PDF",
        downloadWord: "Word (.docx)",
      },
      questionPaper: {
        totalMarks: (n: number) => `${n} marks`,
        duration: (n: number) => `${n} min`,
        instructions: "Instructions",
        marksLabel: (n: number) => `${n} mark${n === 1 ? "" : "s"}`,
        teacher: "Teacher",
        edit: "Edit",
        save: "Save",
        saving: "Saving…",
        cancel: "Cancel",
        saved: "Question paper saved.",
        saveFailed: "Couldn't save your changes.",
        addQuestion: "Add question",
        removeQuestion: "Remove question",
        addOption: "Add option",
        removeOption: "Remove option",
        addSection: "Add section",
        removeSection: "Remove section",
        addInstruction: "Add instruction",
        download: "Download",
        downloadPdf: "PDF",
        downloadWord: "Word (.docx)",
        answerKey: "Answer key",
        answerKeyPdf: "Answer key (PDF)",
        answerKeyWord: "Answer key (Word)",
        answerKeyBuilding: "Building answer key…",
        answerKeyFailed: "Couldn't build the answer key.",
        generalInstructions: "General instructions",
        sectionName: "Section name",
        sectionInstructions: "Section instructions",
        questionText: "Question",
        marks: "Marks",
        type: "Type",
        totalMarksLabel: "Total marks",
        durationMin: "Duration (min)",
        typeMcq: "MCQ",
        typeShort: "Short",
        typeLong: "Long",
      },
      quiz: {
        teacher: "Teacher",
        questionsMeta: (n: number) => `${n} question${n === 1 ? "" : "s"}`,
        minutesMeta: (n: number) => `${n} min`,
        edit: "Edit",
        save: "Save",
        saving: "Saving…",
        cancel: "Cancel",
        saved: "Quiz saved.",
        saveFailed: "Couldn't save your changes.",
        addQuestion: "Add question",
        removeQuestion: "Remove question",
        addOption: "Add option",
        removeOption: "Remove option",
        download: "Download",
        downloadPdf: "PDF",
        downloadWord: "Word (.docx)",
        questionText: "Question",
        options: "Options",
        answer: "Correct answer",
        difficulty: "Difficulty",
        explanation: "Explanation",
        diffEasy: "Easy",
        diffMedium: "Medium",
        diffHard: "Hard",
      },
      notes: {
        summary: "Summary",
        keyPoints: "Key points",
        importantTerms: "Important terms",
      },
    },
  },

  // History page
  history: {
    title: "History",
    subtitle: "View and manage everything you've made in one place.",
    all: "All",
    sortBy: "Sort by",
    sortDate: "Date",
    sortTitle: "Title",
    empty: "Nothing here yet.",
    failed: "Couldn't load your history.",
    loading: "Loading…",
    loadMore: "Load more",
  },

  notifications: {
    title: "Notifications",
    empty: "No notifications yet.",
    markAllRead: "Mark all read",
    viewAll: "View all",
    announceTitle: "Send an announcement",
    announceSub: "Reaches everyone in the target audience right away.",
    announceToClass: "Send to your class",
    announceToTeachers: "Send to all teachers",
    announceToStudents: "Send to all students",
    titleLabel: "Title",
    messageLabel: "Message",
    sendBtn: "Send",
    sending: "Sending…",
    sentToast: (n: number) => `Sent to ${n} ${n === 1 ? "person" : "people"}.`,
  },

  homeworkPage: {
    title: "Homework",
    sub: "Assign homework to a class and track who's finished it.",
    studentTitle: "Homework",
    studentSub: "Everything assigned to your class.",
    newBtn: "New homework",
    titleLabel: "Title",
    classLabel: "Class",
    subjectLabel: "Subject (optional)",
    dueLabel: "Due date (optional)",
    descriptionLabel: "Instructions (optional)",
    assignBtn: "Assign",
    assigning: "Assigning…",
    assignedToast: "Homework assigned.",
    empty: "No homework assigned yet.",
    studentEmpty: "No homework right now.",
    doneCount: (done: number, total: number) => `${done}/${total} done`,
    markDone: "Mark as done",
    markUndone: "Mark as not done",
    due: (d: string) => `Due ${d}`,
    noDueDate: "No due date",
  },

  attendancePage: {
    studentTitle: "Attendance",
    studentSub: "Your day-by-day attendance record.",
    studentEmpty: "No attendance recorded yet.",
    presentLabel: "Present",
    absentLabel: "Absent",
  },

  timetablePage: {
    title: "Timetable",
    sub: "Weekly schedule for a class.",
    studentTitle: "Timetable",
    studentSub: "Your class's weekly schedule.",
    classLabel: "Class",
    pickClass: "Pick a class",
    period: (n: number) => `P${n}`,
    empty: "Nothing set for this slot",
    subjectPlaceholder: "Subject",
    saveBtn: "Save changes",
    saving: "Saving…",
    savedToast: "Timetable saved.",
    days: ["Mon", "Tue", "Wed", "Thu", "Fri", "Sat"],
  },

  reportCardPage: {
    title: "Report Cards & Evaluation",
    sub: "Manage student assessments, automated OMR grading, and report cards.",
    studentTitle: "My Report Card",
    studentSub: "Your marks by subject and term performance.",
    pickStudent: "Pick a student",
    selectClass: "Select Class",
    allClasses: "All Assigned Classes",
    searchStudent: "Search student by name or roll number…",
    noClassSelected: "Select a class above to view the student roster and report cards.",
    noStudentsFound: "No students found matching your search.",
    totalStudents: "Total Students",
    avgPerformance: "Class Avg. Score",
    examsConducted: "Exams Conducted",
    createExamBtn: "Create Exam & Scan OMR",
    createExamTitle: "Create Exam & OMR Scanner",
    createExamSub: "Set up a new examination and upload OMR sheets for instant automated evaluation.",
    examNameLabel: "Exam Name",
    examNamePlaceholder: "e.g. Mid-Term Mathematics 2026",
    gradeLabel: "Grade / Class",
    totalMarksLabel: "Total Marks",
    omrUploadLabel: "OMR Answer Sheets (PDF / Image)",
    omrUploadHint: "Drag & drop OMR scan files here, or click to browse",
    processingOmr: "Scanning OMR sheet bubbles & evaluating answers…",
    omrSuccess: "OMR Sheet evaluated successfully! 38/40 questions correctly answered.",
    backToRoster: "Back to Class Roster",
    viewFullReport: "View Report Card",
    studentDetails: "Student Academic Overview",
    overallGrade: "Overall Grade",
    totalScore: "Total Score",
    rankInClass: "Class Rank",
    attendanceRate: "Attendance",
    recentExams: "Recent Exams & Assessments",
    subjectWiseBreakdown: "Subject-Wise Performance",
    addExamAndMarks: "Add Exam & Marks",
    addSubjectToExam: "Add Subject to Exam",
    editMarkTitle: "Edit Subject Mark Entry",
    addMarkTitle: "Add Exam Marks for Student",
    deleteMarkTitle: "Delete Mark Entry?",
    deleteMarkConfirm: "Are you sure you want to delete this mark entry? This action cannot be undone.",
    cancelBtn: "Cancel",
    confirmDeleteBtn: "Delete Entry",
    saveChangesBtn: "Save Changes",
    addEntryBtn: "Add Exam Entry",
    actionsLabel: "Actions",
    cumulativeScore: "Cumulative Score",
    termTotal: "Term Total",
    percentageLabel: "Percentage",
    performanceBar: "Performance Bar",
    officialSummary: "Official Academic Summary",
    studentReport: "Student Report Card",
    inClass: "in class",
    rollNo: "Roll No",
    subjectLabel: "Subject",
    termLabel: "Term",
    marksLabel: "Marks obtained",
    maxMarksLabel: "Out of",
    remarksLabel: "Remarks (optional)",
    saveBtn: "Save mark",
    saving: "Saving…",
    savedToast: "Mark saved.",
    empty: "No marks entered yet.",
    term: "Term",
    subject: "Subject",
    marks: "Marks",

    // Update Marks Workflow
    updateMarksBtn: "Update Marks",
    updateMarksTitle: "Update Marks",
    updateMarksSub: "Enter or upload marks for students in an examination.",
    backToReportCards: "Back to Report Cards",
    examDetailsTitle: "Examination details",
    examLabel: "Examination",
    examPlaceholder: "e.g. Unit Test 1, Mid Term, Half Yearly",
    noClassesAssigned: "No classes are assigned to you yet.",
    noSubjectsAvailable: "No subjects are available for this class.",
    studentMarksHeading: "Student Marks",
    enterMarksSub: "Enter marks for each student.",
    marksEnteredProgress: "Marks entered",
    omrUploadTitle: "Upload OMR",
    omrUploadSub: "Upload an OMR answer sheet to automatically fill marks.",
    omrSupportedFormats: "Supported formats: PDF / JPG / PNG",
    omrUploadBtn: "Upload OMR",
    omrUploaded: "OMR uploaded",
    omrReadyToProcess: "Ready to process",
    saveMarksBtn: "Save marks",
    marksSavedSuccess: "Marks saved successfully.",
  },

  resourcesPage: {
    title: "E-Library",
    sub: "Links to textbooks, videos and reference material, shared by teachers.",
    studentTitle: "E-Library",
    studentSub: "Links your teachers have shared for your class.",
    addBtn: "Add a link",
    titleLabel: "Title",
    urlLabel: "Link (URL)",
    descriptionLabel: "Description (optional)",
    classLabel: "Class (optional)",
    subjectLabel: "Subject (optional)",
    addingBtn: "Adding…",
    addedToast: "Added to the library.",
    deleteConfirm: "Remove this link?",
    deletedToast: "Removed.",
    empty: "Nothing added yet.",
    openLink: "Open",
    allClasses: "All classes",
    allSubjects: "All subjects",
  },

  notesPage: {
    title: "Chapter Notes",
    sub: "Write a short revision note for a chapter -- your students see this.",
    pickChapter: "Pick a chapter",
    summaryLabel: "Summary",
    keyPointsLabel: "Key points (one per line)",
    importantTermsLabel: "Important terms (comma-separated)",
    saveBtn: "Save note",
    saving: "Saving…",
    savedToast: "Note saved.",
    empty: "No note saved for this chapter yet.",
  },

  practicePage: {
    title: "Practice Questions",
    sub: "Add practice questions for a chapter -- your students see these.",
    pickChapter: "Pick a chapter",
    newBtn: "New question",
    questionLabel: "Question",
    typeLabel: "Type",
    typeMcq: "Multiple choice",
    typeShort: "Short answer",
    typeTrueFalse: "True / False",
    optionsLabel: "Options (one per line)",
    answerLabel: "Correct answer",
    difficultyLabel: "Difficulty",
    addBtn: "Add question",
    adding: "Adding…",
    addedToast: "Question added.",
    deleteConfirm: "Remove this question?",
    deletedToast: "Removed.",
    empty: "No practice questions added for this chapter yet.",
  },

  feesPage: {
    title: "Fees",
    sub: "Manually-logged fee payments -- not a payment gateway.",
    studentTitle: "Fees",
    studentSub: "Your fee payment history.",
    pickStudent: "Pick a student",
    amountLabel: "Amount",
    typeLabel: "Fee type",
    dateLabel: "Payment date",
    noteLabel: "Note (optional)",
    logBtn: "Log payment",
    logging: "Logging…",
    loggedToast: "Payment logged.",
    empty: "No payments logged yet.",
    loggedBy: (name: string) => `Logged by ${name}`,
  },

  biharDarpanPage: {
    title: "Bihar Darpan",
    sub: "Learn something about Bihar and its people every day.",
    todayBadge: "Today",
    dayLabel: (n: number) => `Day ${n} of 30`,
    quoteHeading: "Thought of the day",
    affairsHeading: "Bihar current affairs",
    quizHeading: "Today's quiz",
    gkHeading: "Bihar GK",
    quizCorrect: "Correct!",
    quizWrong: "Not quite.",
    quizNext: "Next question",
    browseHeading: "Browse all 30 days",
    disclaimer:
      "Curated for learning. Quotes are compiled from popular sources — teachers should verify wording before classroom use.",
  },

  simulations: {
    title: "Class Simulations",
    sub: "Engage students with hands-on learning experiences",
    all: "All",
    interactiveModule: "Interactive Module",
    notFound: "That simulation isn't available.",
    openOnPhet: "Open on PhET",
    fullscreen: "Fullscreen",
    exitFullscreen: "Exit fullscreen",
    back: "All simulations",
    grades: (a: number, b: number) => `Class ${a}–${b}`,
    attribution:
      "Simulations by PhET Interactive Simulations, University of Colorado Boulder, licensed under CC BY 4.0.",
  },
};

export type Copy = typeof en;
